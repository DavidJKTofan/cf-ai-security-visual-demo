...
export const uc0 = {
  ...
  nodes: [
    ...
    },
    {
      ...
      column: 'center',
      description: 'Aggregates 13 production MCP servers (Backstage, GitLab, Jira, Sentry, Elasticsearch, Prometheus, Google Workspace, internal Release Manager, …) behind a single Cloudflare Access OAuth flow. New stateless MCP SDK v2 servers use createMcpHandler; legacy McpAgent servers are feature-frozen and migrate in stages when they depend on session behavior. Code Mode keeps the model-facing tool surface constant even as upstream tool count grows — critical when 7 sub-reviewers hit the portal concurrently.',
    ...
    },
    {
      ...
      column: 'center',
      description: 'Two governed context layers for agents:\n\n• Engineering Codex (org-wide): 60+ RFCs grouped into owned domains. Requirements use RFC 2119 SHOULD/MUST language, receive stable identifiers, and are extracted into structured JSON for progressive disclosure. Approved RFC findings are advisory; only an enforced MUST can withhold approval or block a merge.\n• AGENTS.md (per-repo): runtime, test commands, conventions, boundaries, dependencies. ~3,900 repos were bootstrapped by a generator using Backstage metadata and repository analysis.\n\nThe same Codex now supports code, technical specification, and incident report reviewers plus mechanical linters and a local OpenCode-based reviewer CLI.',
    },
    {
      id: 'ai-gateway',
      label: 'AI Gateway',
      ...
      column: 'center',
      description: 'Single control plane for all LLM traffic — 20.18M requests / 241.37B tokens per month across the company. BYOK keeps provider keys in Secrets Store; the proxy Worker injects gateway credentials server-side. This internal design deliberately replaces employee identity with an anonymous UUID. For other deployments, AI Gateway can instead sit behind an Access-protected custom domain and attach verified cf.user_id metadata for logs, routing, and spend limits. Cost-based limits support BYOK and Unified Billing when model pricing is known. Prompt/prefix caching supports the reviewer\'s 85.7% cached-token rate; KV-driven model selection and circuit breakers handle live routing changes and provider failures.',
  ...
  steps: [
    ...
    },
    {
      ...
      description: 'Two layers of governed context:\n\n• Per-repo AGENTS.md: runtime, tests, conventions, boundaries, and dependencies; ~3,900 repositories were bootstrapped by a generator using Backstage metadata and repository analysis.\n• Engineering Codex: 60+ domain-owned RFCs written with RFC 2119 SHOULD/MUST statements. A purpose-built agent extracts statements into structured JSON with stable IDs and metadata for lazy discovery. Approved RFC findings are non-blocking; an enforced MUST can withhold approval or block merge.\n\nThe same source now drives the AI code reviewer, spec reviewer, incident report reviewer, TypeScript linters, and a local OpenCode reviewer CLI.',
      why: 'Models without local and authoritative context can produce plausible but noncompliant work. Publicly reported four-month results include nearly 230,000 Codex violations flagged and almost 16,000 that withheld approval; close to 600 open specifications and more than 200 incident reports were also reviewed. The lifecycle separates learning a new standard from enforcing it.',
      activeNodes: ['sub-reviewers', 'codex'],
      activeEdges: ['e-subs-codex'],
      docsUrl: 'https://blog.cloudflare.com/engineering-standards-enforcement/',
    },
    {
      title: 'AI Gateway routes every LLM call with keys, policy, and visibility',
      product: 'Cloudflare AI Gateway',
      description: 'Every sub-reviewer LLM call lands here. Single control plane: 20.18M requests, 241.37B tokens per month across the company.\n\n• BYOK provider keys in Secrets Store, selected through AI Gateway and kept out of app code.\n• This deployment uses an anonymous UUID in cf-aig-metadata so provider-facing logs do not contain employee identity.\n• Other deployments can protect an AI Gateway custom domain with Access; AI Gateway then adds verified cf.user_id metadata for logs, routing, and per-user spend limits.\n• Spend limits are cost-based budgets for BYOK and Unified Billing requests where pricing is known; an exceeded rule returns 429.\n• Per-reviewer model assignments live in Worker + KV configuration, allowing live routing changes.',
      why: 'Identity treatment should match the threat model: Cloudflare\'s reviewer anonymizes employees before provider routing, while an enterprise gateway can retain verified Access identity for investigation and cost governance. Both patterns centralize keys, routing, spend, and retention posture instead of distributing them across clients.',
