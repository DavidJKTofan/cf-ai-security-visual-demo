 ...
 * STT options: Workers AI (Deepgram Flux / Nova-3), Deepgram (third-party)
 * TTS options: Workers AI (Deepgram Aura-1), ElevenLabs (third-party)
...
export const uc11 = {
  ...
  nodes: [
    ...
    },
    {
      ...
      sublabel: 'WorkersAIFluxSTT / Deepgram',
      icon: '\u{1F50A}',
      type: 'cloudflare',
      product: 'Workers AI (Deepgram Flux)',
      column: 'center',
      description: 'A continuous per-call transcriber session handles turn detection and transcription. Built-in provider: WorkersAIFluxSTT (default model @cf/deepgram/flux, purpose-built for conversational turn-taking). WorkersAINova3STT (@cf/deepgram/nova-3) is the recommended transcription-only option. Third-party: @cloudflare/voice-deepgram for the Deepgram API directly. The provider is configured in the withVoice options; switching requires no application code change.',
    ...
    },
    {
      ...
      type: 'cloudflare',
      column: 'center',
      product: 'Workers AI (Deepgram Aura-1 TTS)',
      description: 'LLM output tokens are sentence-chunked and synthesized concurrently — audio for the first sentence begins streaming before the LLM has finished generating. Built-in provider: WorkersAITTS (default model @cf/deepgram/aura-1). Third-party: @cloudflare/voice-elevenlabs for ElevenLabs TTS (including voice cloning). Audio streams back to the client as binary WebSocket frames. If the user interrupts, in-flight TTS is cancelled.',
      docsUrl: 'https://developers.cloudflare.com/agents/api-reference/voice/',
    },
    // Right column — external providers and persistence
    {
      id: 'workers-ai-voice',
      label: 'Workers AI',
      sublabel: 'Deepgram Flux STT + Aura TTS on-network',
      icon: '\u{1F9E0}',
      type: 'cloudflare',
      column: 'right',
      description: 'Workers AI provides both STT (Deepgram Flux / Nova-3 via the @cf/deepgram/* models) and TTS (Deepgram Aura-1) running on Cloudflare\'s GPU network with no additional API keys required. All audio processing stays on Cloudflare\'s network — no data leaves to an external transcription or synthesis service, important for regulated industries.',
      docsUrl: 'https://developers.cloudflare.com/workers-ai/',
    },
    {
      id: 'ext-voice-providers',
      label: 'Third-Party Providers',
      sublabel: 'Deepgram API, ElevenLabs',
      icon: '\u{1F4E1}',
      type: 'ai-service',
      column: 'right',
      description: 'For specialized voice quality or language support, the Voice Agent can call third-party providers directly via @cloudflare/voice-deepgram (DeepgramSTT) and @cloudflare/voice-elevenlabs (ElevenLabsTTS, including voice cloning via voiceId). Switching providers requires only setting the transcriber/tts class property — no pipeline code changes.',
    },
  ],
  ...
  steps: [
    ...
    },
    {
      ...
      product: 'Workers AI (Deepgram Flux)',
      description: 'The Voice Agent streams microphone audio (binary WebSocket frames) to the configured STT provider. A continuous per-call transcriber session handles voice activity detection and turn detection — detecting when the user has finished speaking. WorkersAIFluxSTT (default model @cf/deepgram/flux) is the built-in on-network option with no additional API keys; the @cloudflare/voice-deepgram package is available if you want to call the Deepgram API directly.',
